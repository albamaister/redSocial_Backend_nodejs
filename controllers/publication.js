...
function savePublication(req, res) {
    var params = req.body;
    if ( !params.text ) return res.stattus(200).send({message: 'Debes enviar un texto'});
...
function deletePublication(req, res) {
    ...
    Publication.find({'user': req.user.sub, '_id': publicationId}).remove((err) => {
        ...
        if(!publicationRemoved) return res.status(404).send({message: 'No se a borrado la publicacion'});
        return res.status(200).send({message: 'Pulbicacion eliminada correctamente'});
    })
}
